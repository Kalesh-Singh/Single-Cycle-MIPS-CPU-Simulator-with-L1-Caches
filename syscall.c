...
void print_cpu_stats(){
	...
	printf("\nCache PERFORMANCE: \n");
	printf("\tInstruction Cache Hits = %d\n", processor_stat.ins_cache_hit);
	printf("\tInstruction Cache Misses = %d\n", processor_stat.ins_cache_miss);
	printf("\tInstruction Cache Accesses = %d\n\n", processor_stat.ins_cache_access);
	printf("\tData Cache Misses = %d\n", processor_stat.data_cache_miss);
	printf("\tData Cache Hits = %d\n", processor_stat.data_cache_hit);
	printf("\tData Cache Accesses = %d\n", processor_stat.data_cache_access);
	#endif
	
	printf("\nCPI PERFORMANCE: \n");
	printf("\tCycles = %d\n", processor_stat.cycle);
	printf("\tUseful Instructions = %d\n", processor_stat.useful_ins);
	# if defined (ENABLE_L1_CACHES)
	printf("\tStalls = %d\n", processor_stat.stall);
	#endif
	printf("\tCPI = %0.2f\n", ((float) processor_stat.cycle/(float) processor_stat.useful_ins));
}
